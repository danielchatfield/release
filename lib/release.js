...
function Release(dir, options) {
  ...
  // Attach debugger
  this.debug = require('debug')('release'); 
  ...
  for( var channelName in channels) {
    var channel = new channels[channelName](this.dir);
    if(channel.isPackage()) {
      this.debug("Initialised %s channel", chalk.bold.magenta(channelName));
 ...
 */
Release.prototype.release = function (version, cb) {
  cb = cb || function(){};
  ...
  for (var i = 0; i < this.channels.length; i++) {
    this.channels[i].setVersion(version);
  }
 ...
 */
Release.prototype.currentVersion = function (defaultVersion) {
...
Release.prototype.throwError = function () {
  ...
  console.error('  ' + chalk.styles.red[0] + chalk.styles.bold[0] + 'Error: ' + chalk.styles.bold[1] + msg + chalk.styles.red[1]);
  process.exit(-1);
};
